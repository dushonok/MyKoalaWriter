...
def write_post(post_title,koala_post_type, test = False, callback=print):
    ...
    system_prompt += AI_TXT_GEN_PROPMPTS_BY_TYPE[koala_post_type]["post"]
    ...
    post_txt = send_prompt_to_openai(sys_promt, user_prompt)
    if post_txt["error"] != "":
        raise OpenAIAPIError(f"OpenAI API error: {post_txt['error']} '{post_txt['message']}'")
    
    sys_prompt = SYS_PROMPT_BASE
    system_prompt += AI_TXT_GEN_PROPMPTS_BY_TYPE[koala_post_type]["title"]
    ...
    post_title = send_prompt_to_openai(sys_promt, user_prompt)
    if post_title["error"] != "":
        raise OpenAIAPIError(f"OpenAI API error: {post_title['error']} '{post_title['message']}'")
    
    txt = post_txt['response']
    title = post_title['response']  
    return title, txt 