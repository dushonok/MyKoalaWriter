...
class PostWriter:
    ...
    def _generate_single_post_with_ai(self) -> tuple[str, str]:
        ...
        """
        self.callback("[PostWriter._generate_single_post_with_ai] Generating single post with AI...")
        ...
        self.callback("[PostWriter._generate_single_post_with_ai] Preparing prompts...")
        prompt_config = self._get_single_post_body_prompts(prompt_config)

        self.callback(f"\n[PostWriter._generate_single_post_with_ai] Writing the post body...\n")
        
        if self.test:
            self.callback("[PostWriter._generate_single_post_with_ai] TEST mode: Using mock body")
            ...
            self.callback("[PostWriter._generate_single_post_with_ai] Calling OpenAI API for post body...")
            ...
            self.callback(f"[PostWriter._generate_single_post_with_ai] Post body generated ({len(body)} chars)")
    ...
    def _get_saved_roundup_post(self) -> tuple[str, str]:
        ...
        self.callback(f"[PostWriter._get_saved_roundup_post] Found {len(roundup_items)} roundup items")
        post_items = []
        ...
        if self.test:
            self.callback("[PostWriter._get_saved_roundup_post] TEST mode: Using mock title")
            title = f"[TEST] Top {len(post_items)} {self.post_title}: A Comprehensive Roundup"
        else:
            self.callback("[PostWriter._get_saved_roundup_post] Generating title with AI...")
            title = self._generate_title_with_ai(prompt_config, body_str)

        self.callback("[PostWriter._get_saved_roundup_post] Formatting listicle with WPFormatter...")
        formatted_body = WPFormatter().generate_listicle(post_items)
        self.callback(f"[PostWriter._get_saved_roundup_post] Listicle formatted ({len(formatted_body)} chars)")
